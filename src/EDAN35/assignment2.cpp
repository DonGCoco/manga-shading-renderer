...
edan35::Assignment2::run()
{
	...
	// ---------- NPR / Toon / Outline controls新加的 ----------
	int shading_mode = 0; // 0=Phong, 1=Toon
	int diffuse_bands = 4;
	float specular_threshold = 0.7f;
	float specular_shininess = 32.0f;
	float band_softness = 0.08f;

	int output_mode = 0; // 0=Color, 1=Manga
	int manga_grey_levels = 4;

	bool enable_outline = true;
	float outline_depth_threshold = 0.01f;   
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!shader_reload_failed) {
			...
			for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
				...
				// --- Toon uniforms (Pass 2 lighting) ---
				glUniform1i(glGetUniformLocation(accumulate_lights_shader, "shading_mode"), shading_mode);
				glUniform1i(glGetUniformLocation(accumulate_lights_shader, "diffuse_bands"), diffuse_bands);
				glUniform1f(glGetUniformLocation(accumulate_lights_shader, "specular_threshold"), specular_threshold);
				glUniform1f(glGetUniformLocation(accumulate_lights_shader, "specular_shininess"), specular_shininess);
				glUniform1f(glGetUniformLocation(accumulate_lights_shader, "band_softness"), band_softness);
			...
			glUniform1i(
				glGetUniformLocation(resolve_deferred_shader, "shading_mode"),
				shading_mode
			);
			...
			bind_texture_with_sampler(GL_TEXTURE_2D, 4, resolve_deferred_shader, "normal_gbuffer",
				textures[toU(Texture::GBufferWorldSpaceNormal)], samplers[toU(Sampler::Nearest)]);

			bind_texture_with_sampler(GL_TEXTURE_2D, 5, resolve_deferred_shader, "depth_gbuffer",
				textures[toU(Texture::DepthBuffer)], samplers[toU(Sampler::Nearest)]);

			// --- Output mode uniforms (Resolve pass) ---
			glUniform1i(glGetUniformLocation(resolve_deferred_shader, "output_mode"), output_mode);
			glUniform1i(glGetUniformLocation(resolve_deferred_shader, "manga_grey_levels"), manga_grey_levels);

			glUniform1i(glGetUniformLocation(resolve_deferred_shader, "enable_outline"), enable_outline ? 1 : 0);
			glUniform1f(glGetUniformLocation(resolve_deferred_shader, "outline_depth_threshold"), outline_depth_threshold);
			glUniform1f(glGetUniformLocation(resolve_deferred_shader, "outline_normal_threshold"), outline_normal_threshold);
			glUniform1f(glGetUniformLocation(resolve_deferred_shader, "outline_strength"), outline_strength);
		...
		if (opened) {
			...
			ImGui::Combo("Shading mode", &shading_mode, "Phong\0Toon\0");
			ImGui::SliderInt("Diffuse bands", &diffuse_bands, 2, 8);
			ImGui::SliderFloat("Band softness", &band_softness, 0.0f, 0.2f);
			ImGui::SliderFloat("Spec threshold", &specular_threshold, 0.0f, 1.0f);
			ImGui::SliderFloat("Spec shininess", &specular_shininess, 1.0f, 256.0f, "%.0f");

			ImGui::Separator();
			ImGui::Text("Outline / Manga");

			ImGui::Checkbox("Enable outline", &enable_outline);
			ImGui::SliderFloat("Outline depth th", &outline_depth_threshold, 0.0001f, 0.05f, "%.5f");
			ImGui::SliderFloat("Outline normal th", &outline_normal_threshold, 0.0f, 1.5f);
			ImGui::SliderFloat("Outline strength", &outline_strength, 0.0f, 3.0f);

			ImGui::Combo("Output mode", &output_mode, "Color\0Manga\0");
			ImGui::SliderInt("Manga grey levels", &manga_grey_levels, 2, 8);
...
namespace
{
	...
	FBOs createFramebufferObjects(Textures const& textures)
	{
		...
		glDrawBuffer(GL_NONE);  // 添加这行：不需要颜色缓冲
		glReadBuffer(GL_NONE);  // 添加这行：不需要读取颜色

	...
	bool generate_mipmap)
{
	...
	//  Generate and bind a cubemap texture object
	glGenTextures(1, &texture); 
	assert(texture != 0u);
	glBindTexture(GL_TEXTURE_CUBE_MAP, texture);

	// Set basic parameters for cubemap
	...
	// Load six faces of the cubemap
	struct Face { GLenum target; std::string const& path; };
	...
	std::uint32_t base_w = 0, base_h = 0;
	bool first = true;


	for (auto const& f : faces) {
		std::uint32_t w = 0, h = 0;
		auto data = getTextureData(f.path, w, h, /*flip=*/false);
		if (data.empty()) {
		...
		// Make sure all six faces have the same resolution
		if (first) {
			base_w = w; base_h = h; first = false;
		}
		else if (w != base_w || h != base_h) {
			LogWarning("Cubemap face size mismatch: expected %ux%u but got %ux%u for %s",
				base_w, base_h, w, h, f.path.c_str());
			glBindTexture(GL_TEXTURE_CUBE_MAP, 0u);
			glDeleteTextures(1, &texture);
			return 0u;
		}

		// Upload this face's image to GPU memory
		glTexImage2D(f.target,
			/*level*/0,
			/*internal*/GL_RGBA,
